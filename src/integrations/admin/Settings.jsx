...
import { getAccountDepartments, getAdminUsers, getAuthSession, removeAccountDepartment, setAccountDepartment, updateAdminUser } from '../../auth'
...
export default function Settings() {
  ...
  if (!session || session.user?.role !== 'admin') return <main className="admin-page"><div className="empty-state"><SettingsIcon size={28} /><h2>Acesso restrito</h2><p>Somente o administrador pode acessar as configurações.</p></div></main>

  return <main className="admin-page">
    <header className="admin-header"><div><span className="eyebrow">Configurações</span><h1>Usuários e permissões</h1><p>Controle os acessos e os dispositivos de cada setor.</p></div><a href="/integrations/bitrix/deal" className="admin-back"><ArrowLeft size={15} />Voltar</a></header>
    <nav className="settings-tabs"><button className={tab === 'users' ? 'active' : ''} type="button" onClick={() => setTab('users')}>Usuários</button><button className={tab === 'permissions' ? 'active' : ''} type="button" onClick={() => setTab('permissions')}>Permissões</button></nav>
